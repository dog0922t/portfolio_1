...
const checkImage = () => {
   ...
   if (code) {
      ...
      let qrbangou=code.data.indexOf("?"); //urlの後ろの番号を取得
      const id =code.data.substring(qrbangou); //「？」の開始位置から終了までの文字列を取得。
      const id_kazu =int(id) //文字列を数字に変化
      
      const dataToSend = {"data": "id", "datakazu" :"id_kazu"};
      fetch('/add_data', {
         method: 'POST',
         headers: {
   ...
   .catch(error => {
      console.error(error);
   });//上のコードは、一度も訪れていないリンクならカウントを進め、状態を訪れたにする。
